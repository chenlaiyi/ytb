<template>
  <div class="official-orders">
    <van-nav-bar title="官方订单管理" left-arrow @click-left="$router.go(-1)" />
  ...
  setup() {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        const response = await mallOfficialApi.getOrders(params)
    ...
    const loadStats = async () => {
      try {
        const response = await mallOfficialApi.dashboard()
    ...
    const confirmOrder = async (order) => {
      try {
        ...
        await mallOfficialApi.updateOrderStatus(order.id, { status: 2 })
        showToast('订单确认成功')
    ...
    const shipOrder = async (order) => {
      try {
        ...
        await mallOfficialApi.updateOrderStatus(order.id, { status: 3 })
        showToast('发货成功')
