...
export default {
  ...
  setup() {
    ...
    const overallStats = computed(() => {
      ...
      return statistics.value.official || statistics.value.overall || null
    })
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await mallOfficialApi.getOrders(listQuery)
    ...
    const handleView = async (row) => {
      try {
        const response = await mallOfficialApi.getOrderDetail(row.id)
    ...
    const confirmShip = async () => {
      try {
        ...
        const response = await mallOfficialApi.shipOrder(shipForm.order_id, shipForm)
    ...
    const handleConfirm = async (row) => {
      try {
        ...
        const response = await mallOfficialApi.confirmOrder(row.id)
    ...
    // 标记为自提
    const handleSelfPickup = async (row) => {
      try {
        await ElMessageBox.confirm(
          `确定要将订单"${row.order_no}"标记为自提并完成吗？`,
          '标记为自提',
          {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
        ...
        submitLoading.value = true
        const response = await mallOfficialApi.selfPickupOrder(row.id)

        if (response.code === 0) {
          ElMessage.success('订单已标记为自提并完成')
          fetchData()
          fetchStatistics()
        } else {
        ...
        if (error !== 'cancel') {
          console.error('标记为自提失败:', error)
          ElMessage.error('标记为自提失败：' + (error.response?.data?.message || error.message))
        }
      } finally {
        submitLoading.value = false
      }
    ...
    const handleCancel = async (row) => {
      try {
        ...
        const response = await mallOfficialApi.cancelOrder(row.id, reason)
    ...
    const handleApproveRefund = async (row) => {
      try {
        ...
        submitLoading.value = true
        const response = await mallOfficialApi.approveRefund(row.id)
        ...
        if (error !== 'cancel') {
          ...
          ElMessage.error('退款失败：' + (error.response?.data?.message || error.message))
        }
      } finally {
        submitLoading.value = false
      }
    ...
    const handleRejectRefund = async (row) => {
      try {
        ...
        submitLoading.value = true
        const response = await mallOfficialApi.rejectRefund(row.id, reason)
        ...
        if (error !== 'cancel') {
          ...
          ElMessage.error('拒绝退款失败：' + (error.response?.data?.message || error.message))
        }
      } finally {
        submitLoading.value = false
      }
    ...
    const confirmStatusChange = async () => {
      try {
        ...
        submitLoading.value = true
        const response = await mallOfficialApi.updateOrderStatus(
          statusForm.order_id,
          {
    ...
    const getOrderStatusText = (status) => {
      const texts = {
        0: '待支付',    // STATUS_PENDING_PAYMENT
        1: '待发货',    // STATUS_PENDING_SHIPMENT
        2: '待收货',    // STATUS_PENDING_RECEIVE
        3: '已完成',    // STATUS_COMPLETED
        4: '退款中',    // STATUS_REFUNDING
        5: '已取消'     // STATUS_CANCELLED
      }
      return texts[status] || '未知'
    }
