  ...
  setup() {
    ...
    const submitAddressForm = () => {
      addressFormRef.value.validate((valid) => {
        if (valid) {
          mallOrderApi.updateAddress(order.value.id, addressForm)
            .then(response => {
              if (response.code === 200) {
                ...
                ElMessage.error(response.message || '收货信息更新失败');
              }
            })
            .catch(error => {
    ...
    const submitShipForm = () => {
      shipFormRef.value.validate((valid) => {
        if (valid) {
          mallOrderApi.ship(order.value.id, shipForm)
            .then(response => {
              if (response.code === 200) {
    ...
    const handleConfirm = (id) => {
      ElMessageBox.confirm(
        ...
        mallOrderApi.confirm(id)
          .then(response => {
            if (response.code === 200) {
              ElMessage.success('订单确认完成成功');
              fetchOrderDetail(id);
            } else {
              ElMessage.error(response.message || '订单确认完成失败');
            }
          })
          .catch(error => {
