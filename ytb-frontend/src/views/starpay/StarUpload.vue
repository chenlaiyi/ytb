        ...
        <el-card shadow="hover" class="mt-card upload-card">
          ...
          >
            ...
            <ul class="result-list">
              <template v-if="lastResult.isProcessing">
                <li>文件已上传，系统正在后台解析，请稍后在「上传记录」列表中查看状态。</li>
                <li v-if="lastResult.upload_log_id">记录 ID：{{ lastResult.upload_log_id }}</li>
...
import { uploadStarpayData, fetchStarpayUploadRecords } from '@/api/v1/starpayUpload'
...
const uploadTypes = [
  { value: 'merchants', label: '商户交易明细', icon: Shop },
  { value: 'partners', label: '服务商数据', icon: User }
]
...
const triggerUpload = async () => {
  ...
  formData.append('file_type', uploadForm.type)
  formData.append('type', uploadForm.type)
  const finalFileName = selectedFileName.value || uploadForm.file?.name || `${uploadForm.type || 'starpay'}.xlsx`
  formData.append('file', uploadForm.file, finalFileName)

  uploading.value = true
  try {
    const response = await uploadStarpayData(formData)
    if (response?.code !== 0) {
    ...
    if (payload?.processing) {
      lastResult.value = {
        ...
        message: payload?.message || '文件上传成功，后台正在处理中'
      }
...
const loadRecords = async () => {
  ...
  try {
    const params = {
      ...
      file_type: filters.type || undefined,
      status: filters.status || undefined,
      date_start: filters.dateRange?.[0] || undefined,
      date_end: filters.dateRange?.[1] || undefined
    }
    const response = await fetchStarpayUploadRecords(params)
    if (response?.code !== 0) {
...
const loadCalendarData = async () => {
  ...
  try {
    ...
    while (page <= MAX_CALENDAR_PAGES) {
      const response = await fetchStarpayUploadRecords({ page, per_page: CALENDAR_PAGE_SIZE })
      if (response?.code !== 0) {
