<template>
  <div class="reward-list-page">
    ...
    <!-- 筛选Tab -->
    <van-tabs v-model:active="activeTab" @change="handleTabChange" :sticky="true" color="#ff4a47">
    ...
    <div class="filter-bar">
      <van-dropdown-menu active-color="#ff4a47">
        <van-dropdown-item v-model:value="filterType" :options="typeOptions" />
        <van-dropdown-item v-model:value="filterMonth" :options="monthOptions" />
        <van-dropdown-item v-model:value="filterSort" :options="sortOptions" />
      </van-dropdown-menu>
    </div>
          ...
          >
            ...
            <div class="member-info">
              ...
              <div class="member-date">
                <span>达标时间: {{ formatDate(member.qualify_date) }}</span>
      ...
      <div class="member-detail" v-if="currentMember">
        ...
        <div class="detail-content">
          ...
          <div class="detail-item">
            ...
            <div class="item-value">{{ currentMember.contribution }}</div>
          </div>
          <div class="detail-item">
            <div class="item-label">奖励金额</div>
            <div class="item-value highlight">{{ currentMember.reward_amount }}元</div>
          </div>
          <div class="detail-item">
            <div class="item-label">达标时间</div>
            <div class="item-value">{{ formatDate(currentMember.qualify_date) }}</div>
          </div>
...
import { getBranchRewardList } from '@/api/branchVip';

export default {
  name: 'BranchRewardList',
  setup() {
    ...
    const fetchRewardList = async () => {
      ...
      try {
        ...
        if (!branchInfo) {
          Toast({ type: 'fail', message: '获取分支机构信息失败' });
          return;
        }
        ...
        console.log('获取分支机构奖励名单, 参数:', params);
        const res = await getBranchRewardList(params);
        console.log('分支机构奖励名单API返回结果:', res);
