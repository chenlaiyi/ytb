    ...
    <div class="detail-container" v-if="!loading && dividendDetail">
      ...
      <div class="detail-card">
        ...
        <div class="detail-list">
          ...
          <div class="detail-item">
            <div class="item-label">分支机构</div>
            <div class="item-value">{{ dividendDetail.branch_name || '未知分支' }}</div>
          </div>
          ...
          <div class="detail-item">
            <div class="item-label">本月团队VIP</div>
            <div class="item-value">{{ dividendDetail.month_team_vip_count || 0 }}人</div>
          </div>
          ...
          <div class="detail-item">
            <div class="item-label">本月团队充值</div>
            <div class="item-value">{{ dividendDetail.month_team_device_count || 0 }}台</div>
          </div>
        ...
        <div class="calculation-content">
          ...
          <div class="calculation-steps">
            ...
            <div class="step-item">
              ...
              <span class="step-text">统计{{ dividendDetail.period }}期全平台{{ dividendDetail.type === 'vip_recruitment' ? 'VIP招募' : '充值套餐' }}数据</span>
            </div>
            <div class="step-item">
        ...
        <div class="qualification-content">
          <div class="qualification-item" :class="{ 'active': dividendDetail.level === 'junior' }">
            ...
            <div class="qualification-condition">
              {{ dividendDetail.type === 'vip_recruitment' ? '本月团队新增VIP≥3人' : '本月团队新增充值≥10台' }}
            </div>
          </div>
          ...
          <div class="qualification-item" :class="{ 'active': dividendDetail.level === 'middle' }">
            ...
            <div class="qualification-condition">
              {{ dividendDetail.type === 'vip_recruitment' ? '本月团队新增VIP≥10人（且本月有直推）' : '本月团队新增充值≥30台（且本月有直推）' }}
            </div>
          </div>
          ...
          <div class="qualification-item" :class="{ 'active': dividendDetail.level === 'senior' }">
            ...
            <div class="qualification-condition">
              {{ dividendDetail.type === 'vip_recruitment' ? '本月团队新增VIP≥30人（且本月有直推）' : '本月团队新增充值≥80台（且本月有直推）' }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="platform-info-card">
        <div class="card-title">平台分红说明</div>
        <div class="platform-info-content">
          <div class="info-text">
            • 平台分红基于全平台VIP和充值数据计算<br>
            • 分支机构用户参与全平台分红池分配<br>
            • 达标条件基于用户在分支机构内的团队表现<br>
            • 分红金额由平台统一计算和发放
          </div>
        </div>
      </div>
...
import { getBranchVipDividendDetail } from '@/api/branchVip'
...
const fetchDividendDetail = async () => {
  try {
    ...
    if (!userId || !branchCode) {
      Toast({ type: 'fail', message: '用户信息不完整，请重新登录' })
      router.push('/branch-login')
      return
    }
    
    // 调用真实API获取分红详情
    const response = await getBranchVipDividendDetail(dividendId, {
      user_id: userId,
    ...
    if (response.code === 0) {
      ...
      Toast({ type: 'fail', message: response.message || '获取分红详情失败' })
    }
