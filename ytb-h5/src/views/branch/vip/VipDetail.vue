<template>
  <div class="vip-detail-container">
    <van-nav-bar title="VIP会员详情" left-arrow @click-left="onClickLeft" />
...
import { getBranchVipUserDetail, getBranchVipUserDividends, getBranchVipUserTeam } from '@/api/branchVip';

export default {
  name: 'BranchVipDetail',
  setup() {
    ...
    const fetchUserDetail = async () => {
      try {
        ...
        const response = await getBranchVipUserDetail(userId, branchInfo.branch_code);
    ...
    const fetchDividends = async () => {
      try {
        ...
        const response = await getBranchVipUserDividends({
          user_id: userId,
    ...
    const fetchTeamMembers = async () => {
      try {
        ...
        const response = await getBranchVipUserTeam({
          user_id: userId,
          branch_code: branchInfo.branch_code,
          page: 1,
          page_size: 5
    ...
    const viewAllDividends = () => {
      router.push(`/branch/vip/dividend-history?user_id=${userId}`);
    };
    
    const viewAllTeamMembers = () => {
      router.push(`/branch/vip/team?user_id=${userId}`);
    };
